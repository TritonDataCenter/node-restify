...
function HttpClient(options) {
    ...
    /*
     * Which HTTP proxy envvars to support, in which order of preference and
     * for proxying to which protocol varies widely. Some notes:
     *
     * - `curl` strictly separates http_proxy for HTTP and https_proxy for
     *   HTTPS. It supports "ALL_PROXY" and "[protocol]_PROXY".
     * - The "request" module allows http_proxy as a fallback for HTTPS. It
     *   does not support "ALL_PROXY".
     *
     * Restify is all about HTTP(S), so we won't currently support ALL_PROXY.
     * As well, this impl is a mix of what you see in `curl` and "request".
     * If an application wants different behaviour, it can handle envvars
     * itself.
     *
     * TODO: NO_PROXY support.
     */
        ...
        // Assume 'http:'.
        this.proxy = proxyOptsFromStr(process.env.http_proxy ||
            process.env.HTTP_PROXY);
    }
...
HttpClient.prototype.close = function close() {
    /*
     * The node (at least v0.10) default agent has:
     *      <agent>.sockets = <mapping of hostname to array of socket objects>
     * and, apparently, either `<agent>.idleSockets` and `<agent>.freeSockets`.
     *
     ...
     *
     *
     * I'm not sure what necessitated this restify client working on
     * `idleSockets` and `freeSockets`, but it is lame that it needs to
     * dive into undocumented fields.
     *
     * Either the following from the node docs is misleading and/or wrong or
     * I'm missing something about the need to call `.close()` to be able
     * to exit the node process immediately:
     *
     * [https://nodejs.org/docs/v0.10.30/api/all.html#all_class_http_agent]
     * > If no pending HTTP requests are waiting on a socket to become
     * > free the socket is closed. This means that node's pool has the
     * > benefit of keep-alive when under load but still does not require
     * > developers to manually close the HTTP clients using keep-alive.
     *
     * IMO, the right answer is that node's Agent class should document and
    ...
    if (this.agent) {
        ...
        if (Array.isArray(sockets)) {
            ...
            Object.keys((sockets || {})).forEach(function (k) {
                if (Array.isArray(sockets[k])) {
                    self.log.trace({host: k}, 'ending %d sockets', sockets[k].length);
        ...
        Object.keys((sockets || {})).forEach(function (k) {
            self.log.trace({host: k}, 'ending idle/free %d sockets', sockets[k].length);
            sockets[k].forEach(function (s) {
                s.end();
            });
        });
